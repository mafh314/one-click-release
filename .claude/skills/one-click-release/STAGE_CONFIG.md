...
**Inputs:** `VERSION`, `MAJOR_MINOR`, `MM_DASHED`, `RELEASE_BRANCH`, `CURRENT_RELEASE_TAG`, `KONFLUX_NS`, `KONFLUX_SERVER`, `KONFLUX_TOKEN`, `GITLAB_URL`, `GITLAB_TOKEN`, `GITLAB_TOKEN_WRITEABLE`, `TZ_FMT`

**Constraints:**
- Konflux cluster: **READ-ONLY** for verify commands (`oc get`/`kubectl get` only)
- GitLab: **READ-ONLY** for verify commands (`GET` requests only); execute commands for steps 1.5, 1.6, 1.11, and 1.12 may write to GitLab when `GITLAB_TOKEN_WRITEABLE=true`
...
## GitLab MR Creation Pattern

Steps 1.5, 1.6, 1.11, and 1.12 share the same three-step pattern to create a GitLab MR. Each execute path checks `GITLAB_TOKEN_WRITEABLE` first — if false, it prints an error and falls back to MANUAL instructions.

**Branch naming convention:** `auto/{MM_DASHED}/{BRANCH_SUFFIX}` (e.g. `auto/1-21/rpa-config`)
...
Each step sets `GITLAB_PROJECT` and `BRANCH_SUFFIX` before calling the pattern:
...
# Step 1.6:
GITLAB_PROJECT="releng%2Fpyxis-repo-configs"
```

`BRANCH_SUFFIX` is set per step (see table above), e.g.:
```bash
BRANCH_SUFFIX="rpa-config"   # step 1.5
BRANCH_SUFFIX="pyxis-config" # step 1.6
BRANCH_SUFFIX="product-version" # step 1.11
BRANCH_SUFFIX="cdn-rp-rpa"  # step 1.12
```
...
curl -s -X POST \
  ...
  --data "{
    \"branch\": \"auto/${MM_DASHED}/${BRANCH_SUFFIX}\",
    \"ref\": \"main\"
  }"
```
...
Uses the [GitLab Commits API](https://docs.gitlab.com/ee/api/commits.html) `actions` array, which supports creating multiple files in a single commit.
...
For binary or YAML content with special characters, use `base64` encoding:
```json
{
  "action": "create",
...
Multiple files: add one object per file to the `actions` array. All files are committed atomically in a single commit.
...
curl -s -X POST \
  ...
  --data "{
    \"source_branch\": \"auto/${MM_DASHED}/${BRANCH_SUFFIX}\",
    \"target_branch\": \"main\",
    \"title\": \"{MR_TITLE}\",
...
- A note that the MR was auto-generated by the one-click-release skill

### Fallback

Every execute path in steps 1.5, 1.6, 1.11, and 1.12 starts with:

```bash
if [ "$GITLAB_TOKEN_WRITEABLE" != "true" ]; then
  echo "ERROR: GITLAB_TOKEN requires 'api' scope to auto-create this MR."
  echo "  Falling back to MANUAL. Provision a token with 'api' scope to enable auto-creation."
  # ... existing MANUAL instructions follow
fi
```
